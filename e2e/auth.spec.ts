...
test.describe("auth lifecycle", () => {
  ...
  test("register creates account, signs in, and lands on home", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /audit npm libraries/i }),
    ).toBeVisible();
  });
  ...
  test("wrong password shows error banner and stays on login", async ({
    page,
  }) => {
    const account = await registerAndLogin(page);
    await page.getByRole("button", { name: account.username }).click();
