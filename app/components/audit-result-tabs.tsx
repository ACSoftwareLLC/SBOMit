          ...
          <p className="text-sm text-muted-foreground">
            {result.competitionReadout ? (
              <>
                ...
                {result.competitionReadout.judge.model}. Model A and B reports
                are available in the Competition tab.
              </>
            ) : (
              <>
                Generated by{" "}
                {activeJob?.interactions?.[0]?.provider
                  ? (providerLabels[
                      activeJob.interactions[0].provider as
                        | "openai"
                        | "anthropic"
                        | "google"
                    ] ?? activeJob.interactions[0].provider)
      ...
      <Tabs value={activeTab} onValueChange={onTabChange}>
                ...
                <p className="text-sm text-muted-foreground">
                  {(() => {
                    ...
                    return [
                      ...
                      output > 0 ? `${output.toLocaleString()} out` : null,
                    ]
                      .filter(Boolean)
                      .join(" / ");
                  })()}
          ...
          {result.investigationAreas.length === 0 ? (
            <Card>
              <CardContent className="py-8 text-center text-sm text-muted-foreground">
                No investigation areas identified. This may happen when the
                source code was not available for inspection.
              </CardContent>
            </Card>
          ) : (
        ...
        <TabsContent value="deep-dive" className="space-y-4">
          {result.deepDiveFindings.length === 0 ? (
            <Card>
              <CardContent className="py-8 text-center text-sm text-muted-foreground">
                No file-level findings. Either no issues were found or the
                ...
                <CardHeader>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <CardTitle className="text-lg">{finding.file}</CardTitle>
                    </div>
